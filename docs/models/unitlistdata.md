...
| `serial_number` | `std::string` | :heavy_check_mark: | Human-readable serial number assigned to the unit. |
| `created_at` | `std::string` | :heavy_check_mark: | ISO 8601 timestamp when the unit was created. |
| `sample` | `std::optional<Sample>` | :heavy_minus_sign: | Reference-sample classification. 'golden' = known-good reference, 'failing' = known-faulty reference, 'ignored' = bench-check unit excluded from analytics and alerts, null = production unit. |
...
| `metadata` | `std::optional<std::map<std::string, nlohmann::json>>` | :heavy_minus_sign: | Custom metadata key/value pairs on the unit. Only present when the request sets `include_metadata=true`. |
