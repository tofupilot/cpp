...
| `message` | `std::string` | :heavy_check_mark: | Content of the log message. Contains the actual log text describing the event, error, or information being logged. Messages longer than 10,000 characters will be truncated; empty messages become "(empty)". |
| `source_file` | `std::string` | :heavy_check_mark: | Name or path of the source file where the log message originated. Helps identify the code location that generated the log entry. Paths longer than 200 characters keep the trailing 200 (leading characters are dropped); empty values become "unknown". |
