...
| `outcome` | `std::optional<ValidatorsOutcome>` | :heavy_minus_sign: | Aggregation validation result: PASS, FAIL, UNSET, or null if no validators. |
...
| `validators` | `NullableField<std::vector<RunGetAggregationsValidators>>` | :heavy_minus_sign: | Validators applied to the aggregated value. |
