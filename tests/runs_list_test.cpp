...
TEST(RunsList, Pagination) {
    ...
    for (int i = 0; i < 3; ++i) { auto ts = iso_now(); client().runs().create().serial_number("SN-PG-" + std::to_string(i) + "-" + uid_val).procedure_id(procedure_id()).part_number(part).outcome(LogGetOutcome::Pass).started_at(ts).ended_at(ts).send(); }
...
TEST(RunsList, FilterByOutcome) {
    ...
    auto pass_run = client().runs().create().serial_number("SN-P-" + uid_val).procedure_id(procedure_id()).part_number(part).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
    client().runs().create().serial_number("SN-F-" + uid_val).procedure_id(procedure_id()).part_number(part).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Fail).send();
    auto result = client().runs().list().outcomes({LogGetOutcome::Pass}).part_numbers({part}).send();
    ASSERT_FALSE(result.data.empty());
    for (const auto& r : result.data) EXPECT_EQ(LogGetOutcome::Pass, r.outcome);
}
...
TEST(RunsList, FilterBySerialNumber) {
    ...
    auto ts = iso_now();
    client().runs().create().serial_number(serial).procedure_id(procedure_id()).part_number("PART-" + uid_val).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, FilterByProcedureId) {
    ...
    auto ts = iso_now();
    client().runs().create().serial_number("SN-FPI-" + uid_val).procedure_id(proc.id).part_number("PART-FPI-" + uid_val).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, SortOrder) {
    ...
    auto ts = iso_now();
    for (int i = 0; i < 2; ++i)
        client().runs().create().serial_number("SN-SORT-" + std::to_string(i) + "-" + uid_val).procedure_id(procedure_id()).part_number(part).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, FilterByDurationRange) {
    ...
    client().runs().create().serial_number("SN-DUR-" + uid_val).procedure_id(procedure_id()).part_number(part).started_at(started).ended_at(ended).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, FilterByEndedAt) {
    ...
    client().runs().create().serial_number("SN-END-" + uid_val).procedure_id(procedure_id()).part_number(part).started_at(started).ended_at(ended).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, FilterByRevisionNumbers) {
    ...
    client().runs().create().serial_number("SN-REV-" + uid_val).procedure_id(procedure_id()).part_number(part).revision_number(rev).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, FilterByProcedureVersions) {
    ...
    auto ts = iso_now();
    client().runs().create().serial_number("SN-PV-" + uid_val).procedure_id(procedure_id()).part_number(part).procedure_version(version).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
...
TEST(RunsList, FilterByBatchNumbers) {
    ...
    auto ts = iso_now();
    client().runs().create().serial_number("SN-BN-" + uid_val).procedure_id(procedure_id()).part_number(part).batch_number(batch).started_at(ts).ended_at(ts).outcome(LogGetOutcome::Pass).send();
