...
inline void to_json(nlohmann::json& j, const ProcedureGetCreatedByUser& v) {
...
inline void from_json(const nlohmann::json& j, ProcedureGetCreatedByUser& v) {
    if (!j.contains("id")) {
...
class ProcedureGetCreatedByUserBuilder {
public:
    /// Set the `id` field.
    /// User ID.
    ProcedureGetCreatedByUserBuilder& id(std::string value) {
        id_ = std::move(value);
        return *this;
    }
    ...
    ProcedureGetCreatedByUser build() const& {
        ProcedureGetCreatedByUser result;
        if (!id_.has_value()) {
    ...
    ProcedureGetCreatedByUser build() && {
        ProcedureGetCreatedByUser result;
        if (!id_.has_value()) {
            throw std::runtime_error("missing required field: id");
        }
