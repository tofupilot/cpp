// Auto-generated by TofuPilot SDK Generator. DO NOT EDIT.

#pragma once

#include <cstdint>
#include <map>
#include <optional>
#include <stdexcept>
#include <string>
#include <utility>
#include <vector>

#include <nlohmann/json.hpp>


namespace tofupilot {

/// User who created the deployment. Null for system-created deployments.
struct DeploymentGetCreatedByUser {
    std::string id;
    std::string name;
    std::optional<std::string> image;
};

inline void to_json(nlohmann::json& j, const DeploymentGetCreatedByUser& v) {
    j = nlohmann::json::object();
    j["id"] = v.id;
    j["name"] = v.name;
    if (v.image.has_value()) {
        j["image"] = v.image.value();
    }
}

inline void from_json(const nlohmann::json& j, DeploymentGetCreatedByUser& v) {
    if (!j.contains("id")) {
        throw nlohmann::json::other_error::create(501,
            "missing required field in response: id", &j);
    }
    v.id = j["id"].get<std::string>();
    if (!j.contains("name")) {
        throw nlohmann::json::other_error::create(501,
            "missing required field in response: name", &j);
    }
    v.name = j["name"].get<std::string>();
    if (j.contains("image") && !j["image"].is_null()) {
        v.image = j["image"].get<std::string>();
    }
}

/// Builder for DeploymentGetCreatedByUser.
class DeploymentGetCreatedByUserBuilder {
public:
    /// Set the `id` field.
    DeploymentGetCreatedByUserBuilder& id(std::string value) {
        id_ = std::move(value);
        return *this;
    }

    /// Set the `name` field.
    DeploymentGetCreatedByUserBuilder& name(std::string value) {
        name_ = std::move(value);
        return *this;
    }

    /// Set the `image` field.
    DeploymentGetCreatedByUserBuilder& image(std::string value) {
        image_ = std::move(value);
        return *this;
    }

    /// Build the struct. Throws std::runtime_error if required fields are missing.
    DeploymentGetCreatedByUser build() const& {
        DeploymentGetCreatedByUser result;
        if (!id_.has_value()) {
            throw std::runtime_error("missing required field: id");
        }
        result.id = id_.value();
        if (!name_.has_value()) {
            throw std::runtime_error("missing required field: name");
        }
        result.name = name_.value();
        result.image = image_;
        return result;
    }

    /// Build the struct (move overload). Throws std::runtime_error if required fields are missing.
    DeploymentGetCreatedByUser build() && {
        DeploymentGetCreatedByUser result;
        if (!id_.has_value()) {
            throw std::runtime_error("missing required field: id");
        }
        result.id = std::move(id_.value());
        if (!name_.has_value()) {
            throw std::runtime_error("missing required field: name");
        }
        result.name = std::move(name_.value());
        result.image = std::move(image_);
        return result;
    }

private:
    std::optional<std::string> id_;
    std::optional<std::string> name_;
    std::optional<std::string> image_;
};

} // namespace tofupilot
